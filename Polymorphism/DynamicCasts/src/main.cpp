...
#include "dog.hpp"  // it will include its parents
...
// Dynamic cast are meant to cast from base information to derived information
// and input and output must be part of the same polymorphic inheritance
// hierarchy.
int main() {
  // Base class pointer
  ...
  if (feline_ptr) {
    ...
    std::cout << "Couldn't do the transformation from Animal* to Dog*"
              << std::endl;
  }
  ...
  // make sure that you use dynamic cast in an inheritance hierarchy that
